...
@implementation API_user_interest
...
-(BOOL)is_invalid
{
    ...
    if(![API_Base is_object_valid:self.interests]) {
        return true;
    }
...
+(API_user_interest *) create:(id)view_model user_id:(id)user_id interests:(id)interests {
    ...
    API_user_interest *model = [[self alloc] init];
    if( !model ) {
        return nil;
    }
...
-(BOOL)build
{
    ...
    [self.param_table addObject:self.interests?self.interests:@""];
