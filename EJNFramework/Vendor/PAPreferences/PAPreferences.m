...
- (instancetype)init {
    if (self = [super init]) {
        ...
        for (int i=0; i<cProps; i++) {
            ...
            if (isDynamic) {
                if (shouldRetain) {
                    DDLogVerbose(@"Retained properties are not supported by PAPreferences, use assign instead.");
                } else {
                    if ([self isValidType:type]) {
                        ...
                        DDLogVerbose(@"Type of %@ is not supported by PAPreferences.", name);
                    }
                }
            }
        }
...
- (BOOL)synchronize {
    ...
    if (!result) {
        DDLogWarn(@"Warning - Failed to synchronise user defaults, some of the data may be stale.");
    }
    return result;
}
