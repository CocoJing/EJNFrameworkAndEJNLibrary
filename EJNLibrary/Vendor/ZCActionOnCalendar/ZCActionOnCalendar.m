...
+ (void)saveEventStartDate:(NSDate*)startData endDate:(NSDate*)endDate alarm:(float)alarm
                eventTitle:(NSString*)eventTitle location:(NSString*)location isReminder:(BOOL)isReminder{
    //事件市场
    ...
    if ([eventStore respondsToSelector:@selector(requestAccessToEntityType:completion:)]) {
        ...
        [eventStore requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (error) {
                    DDLogWarn(@"日历添加失败1");
                    [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Notice",nil)
                                                message:NSLocalizedString(@"Add to the calendar error 1", nil)
                                               delegate:nil
                    ...
                    //被⽤用户拒绝,不允许访问⽇日历
                    [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Notice",nil)
                                                message:NSLocalizedString(@"We don't have access to the calendar.", nil)
                                               delegate:nil
                                      cancelButtonTitle:NSLocalizedString(@"OK", nil)
                                      otherButtonTitles:nil] show];
                }
                    ...
                    [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Notice",nil)
                                                message:NSLocalizedString(@"Access to the calendar Success", nil)
                                               delegate:nil
                                      cancelButtonTitle:NSLocalizedString(@"OK", nil)
                                      otherButtonTitles:nil] show];
                    ...
                    if (isReminder) {
                        ...
                        if (error) {
                            DDLogWarn(@"日历添加失败 2 %@", [error localizedDescription]);
                        }
                    }
                }
            });
        ...
        if (isReminder) {
            ...
            if (error) {
                [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Notice",nil)
                                            message:NSLocalizedString(@"Add to the calendar error", nil)
                                           delegate:nil
                                  cancelButtonTitle:NSLocalizedString(@"OK", nil)
                                  otherButtonTitles:nil] show];
            }
